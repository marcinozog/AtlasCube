...
- Volume curve: the 0–100 % position is raised to an exponent before it becomes
  ...
  taper that keeps the quiet end of the scale usable; `false` = 2.0, half as
  steep in dB, for amps that already taper in hardware. Both reach full scale at
  100 %; the gentle leg is louder below that (20 % is −28 dB against −56 dB).
  A truly linear leg (exponent 1.0) was tried and rejected — at −14 dB on 20 %
  it made the usable range a sliver at the bottom of the dial. Lives in
  `audio_engine_set_volume_log()`, so every path that sets a level — encoder,
  web, MQTT, radio fade-in, notification volume — inherits it. Bluetooth is
  outside the DSP and scales in the module instead.
