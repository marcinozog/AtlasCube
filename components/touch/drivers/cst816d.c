#include "cst816d.h"

#include "driver/gpio.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "esp_log.h"

static const char *TAG = "CST816D";

#define CST816D_I2C_ADDR        0x15
#define CST816D_I2C_FREQ_HZ     400000
#define CST816D_REG_TOUCH_BASE  0x01    // gesture, points, x_hi, x_lo, y_hi, y_lo

static i2c_master_dev_handle_t s_dev = NULL;

static void cst816d_reset(int rst_gpio)
{
    if (rst_gpio < 0) return;

    gpio_set_level(rst_gpio, 0);
    vTaskDelay(pdMS_TO_TICKS(10));
    gpio_set_level(rst_gpio, 1);
    vTaskDelay(pdMS_TO_TICKS(50));
}

void cst816d_init(i2c_master_bus_handle_t bus, int rst_gpio)
{
    cst816d_reset(rst_gpio);

    const i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address  = CST816D_I2C_ADDR,
        .scl_speed_hz    = CST816D_I2C_FREQ_HZ,
    };

    esp_err_t err = i2c_master_bus_add_device(bus, &dev_cfg, &s_dev);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "i2c_master_bus_add_device failed: %s", esp_err_to_name(err));
        s_dev = NULL;
        return;
    }

    ESP_LOGI(TAG, "Initialized at 0x%02X", CST816D_I2C_ADDR);
}

bool cst816d_read(uint16_t *x, uint16_t *y)
{
    if (s_dev == NULL) return false;

    uint8_t reg = CST816D_REG_TOUCH_BASE;
    uint8_t buf[6] = {0};

    esp_err_t err = i2c_master_transmit_receive(s_dev, &reg, 1, buf, sizeof(buf), 50);
    if (err != ESP_OK) return false;

    // buf[0] = gesture, buf[1] = num_points, buf[2..5] = xH, xL, yH, yL
    if (buf[1] == 0) return false;

    uint16_t rx = ((uint16_t)(buf[2] & 0x0F) << 8) | buf[3];
    uint16_t ry = ((uint16_t)(buf[4] & 0x0F) << 8) | buf[5];

    if (x) *x = rx;
    if (y) *y = ry;
    return true;
}
