...
| `GET /api/diag` | Read-only health snapshot for bug reports, grouped: `fw` (`version`, `build`, `idf`, `variant`, `update_available`, `update_latest`), `www` (`outdated`, `version`, `expected`), `psram` and `internal` (`total`, `free`, `min_free`; `internal` adds `largest`), `hw` (`chip`, `revision`, `cores`, `flash_size`, `panel_w`, `panel_h`), `net` (`connected`, `ssid`, `ip`, `rssi`, `mac`), `sd` (`mounted`, `total`, `free` — zeros unless the card is already mounted), `cpu` (`core0`, `core1`, % **since the previous GET**, so the first read returns 0) and `uptime_s`. Byte counts are bytes. Same data as the on-device Settings → System → Diagnostics screen. |
