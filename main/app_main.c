...
void app_main(void)
{
    ...
    // Auto-update: background check (STA only — needs internet). Firmware version
    // first; when it's current, the same task compares the web-UI stamp locally
    // (an app-only update never rewrites the www partition). The check ALWAYS
    // runs (its server check-in is how device usage is counted);
