 ...
 *
 * Compiled out entirely without HAS_ESPNOW_PILOT (defines.h): every function
 * below becomes a stub, so callers need no #ifdef of their own — they read
 * `supported` from the status snapshot.
 */

/**
 * Brings the link up if — and only if — a pilot is already paired. On a radio
 ...
 * pilot sleeps between button presses, so silence for hours is a healthy pilot,
 * not a fault — `last_seen_s` is the honest reading and a UI should show the age,
 * not a verdict.
 */
typedef struct {
    bool     supported;                     ///< false when built without HAS_ESPNOW_PILOT
    bool     started;                       ///< esp_now_init() has run
    bool     paired;                        ///< a pilot MAC is stored
    ...
    uint32_t rx_frames;                     ///< frames accepted from the pilot since boot
    uint32_t tx_ok;                          ///< replies the pilot's MAC ACKed
