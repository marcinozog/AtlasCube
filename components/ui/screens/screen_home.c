...
static void home_create(lv_obj_t *parent)
{
    ...
    // The strip also anchors the station/title labels (align_to base), so it
    // must always exist. clock_show_strip controls only its background plate.
    ...
    // Station/title live on the screen, not inside the strip: LVGL clips
